...
?>
<div id="<?php echo esc_attr($html_id) ?>" class="ct-fancy-box ct-fancy-box-layout8 <?php echo esc_attr($settings['style_l8'].' '.$settings['ct_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['ct_animate_delay']); ?>ms">
            ...
            <?php if(!empty($settings['btn_text'])) : ?>
                <div class="item--button">
                    <a <?php ct_print_html($widget->get_render_attribute_string( 'btn_link' )); ?> ><?php echo esc_attr($settings['btn_text']); ?>
                        <i class="more-icon-hover flaticon-right-arrow"></i>
                    </a>
                </div>
            <?php endif; ?>
        </div>
