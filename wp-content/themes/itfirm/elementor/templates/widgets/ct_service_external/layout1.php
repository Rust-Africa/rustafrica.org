...
<?php if(isset($settings['service']) && !empty($settings['service']) && count($settings['service'])): ?>
    <div id="<?php echo esc_attr($html_id); ?>" class="ct-service-external ct-service-external1 ct-slick-slider slick-boxshadow slick-dots-line1 slick-arrows-1">
        <div class="ct-inline-css"  data-css="
            <?php if( !empty($settings['color_gr_from']) && !empty($settings['color_gr_to']) ) : ?>
                #<?php echo esc_attr($html_id) ?>.ct-service-external1 .item--icon::before,
                #<?php echo esc_attr($html_id) ?>.ct-service-external1 .item--title::before,
                #<?php echo esc_attr($html_id) ?>.ct-service-external1 .btn-text-gr,
                #<?php echo esc_attr($html_id) ?>.ct-service-external1 .item--inner:hover .item--icon i,
                #<?php echo esc_attr($html_id) ?>.ct-service-external1 .item--overlay {
                                ...
                                <div class="item--holder">
                                    ...
                                    <?php if(!empty($btn_text)) : ?>
                                        <div class="item--readmore">
                                            <a class="btn-text-gr" <?php echo implode( ' ', [ $link_attributes ] ); ?>><?php echo esc_attr($btn_text); ?><i class="flaticon-right pxl-rtl-icon"></i></a>
                                        </div>
                                    <?php endif; ?>
                                </div>
                           </div>
